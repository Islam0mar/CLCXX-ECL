...
inline cl_object box_smart_pointer(PtrT* p)
{
  return boxed_cpp_pointer(p, static_type_mapping<PtrT>::cl_object(), true);
}
...
inline cl_object julia_smartpointer_type()
{
  static cl_object m_ptr_type = (cl_object)_ecl_intern("SmartPointerWithDeref", "CxxWrap");
  return m_ptr_type;
}
...
struct BaseMapping<PtrT<PointeeT, ExtraArgs...>, OtherPtrT>
{
  ...
  struct ConditionalCastToBase
  {
    static void apply()
    {
      ...
      static_type_mapping<typename std::remove_pointer<decltype(ConvertToBase<PtrT<PointeeT>>::apply(std::declval<PtrT<PointeeT>&>()))>::type>::cl_object();
    }
  };
...
inline jl_datatype_t* smart_cl_object()
{
  static jl_datatype_t* wrapped_dt = nullptr;
  static jl_datatype_t* result = nullptr;

  jl_datatype_t* current_dt = static_type_mapping<remove_const_ref<T>>::cl_object();
...
struct SmartJuliaType<PtrT<T>>
{
  ...
  {
    return smart_cl_object<PtrT<T>,PtrT<int>,T>();
  }
};
...
struct SmartJuliaType<std::unique_ptr<T>>
{
  static jl_datatype_t* apply()
  {
    return smart_cl_object<std::unique_ptr<T>,std::unique_ptr<int>,T>();
  }
};
...
struct static_type_mapping<T, typename std::enable_if<IsSmartPointerType<typename std::remove_reference<T>::type>::value>::type>
{
  ...
  static jl_datatype_t* cl_object()
  {
...
{
  cl_object operator()(T cpp_val) const
  {
    return boxed_cpp_pointer(new T(std::move(cpp_val)), static_type_mapping<T>::cl_object(), true);
  }
};
...
{
  T operator()(cl_object julia_val) const
  {
...
struct ConvertToJulia<T&, false, false, false, typename std::enable_if<IsSmartPointerType<T>::value && !std::is_const<T>::value>::type>
{
  cl_object operator()(T& cpp_val) const
  {
    return boxed_cpp_pointer(&cpp_val, static_type_mapping<T>::cl_object(), false);
  }
};
