...
LispFunction::LispFunction(const std::string& name, const std::string& module_name)
{
  ...
  if(!module_name.empty())
  {
    ...
    {
      mod = (cl_object)cl_find_symbol(2,ecl_read_from_cstring(module_name.c_str()), current_mod);
    }
    if(mod == nullptr)
    {
  ...
  // intern modify
  cl_object sym = ecl_make_symbol(name.c_str(), symbol_name(mod).c_str());
